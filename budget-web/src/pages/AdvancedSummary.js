...
function getData(transactions) {
    ...
    const MonthList = {}
    const YearList = {
        [`${year}`]: { "total": 0.00, "category": {} },
        [`${year - 1}`]: { "total": 0.00, "category": {} }
    }

    if (month < 5) {
        for (let i = month; i >= 0; i--) {
            MonthList[`${monthDict[i]} ${year}`] = { "total": 0.00, "category": {} }
        }

        for (let i = 11; i >= 11 - (5 - month) + 1; i--) {
            MonthList[`${monthDict[i]} ${year - 1}`] = { "total": 0.00, "category": {} }
        }
    } else {
    ...
    for (const each of transactions) {
        ...
        if (tmp_format in MonthList) {
            ...
            if (isNaN(MonthList[tmp_format]["category"][each.category])) {
                ...
                const temp = Number(MonthList[tmp_format]["category"][each.category]) + Number(each.value)
        ...
        if (year in YearList) {
            ...
            if (isNaN(YearList[year]["category"][each.category])) {
                ...
                const temp = Number(YearList[year]["category"][each.category]) + Number(each.value)
...
export default function AdvancedSummary() {
    ...
    async function getAllTransactions() {
        try {
            ...
            const { monthList: newMonthList, yearList: newYearList } = getData(transactionsData.transactions);
    ...
    useEffect(() => {
        if (chartRef.current && transaction_list.length > 0) {
            ...
            if (activeTab === 'Monthly') {
                // console.log(monthList)
                data = Object.keys(monthList).map(key => ({
                    "month": key,
                        ...
                        <div class="relative flex items-center font-bold justify-cente ml-4 text-3xl">
                            <div class="group cursor-help">
                                ...
                                <div class="text-4xl absolute top-full mt-2 hidden w-40 rounded bg-black px-3 py-2 text-white text-sm text-center group-hover:block ">
                                    For Monthly graph, only 6-months time windows will be kept; For Yearly graph, only 2-years time windows.
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8 flex flex-col items-center" >
                        ...
                        <div class="border-b border-gray-200 dark:border-gray-700">
                            <ul class="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400  " >
                                ...
                                <li class="me-2">
                                    <a onClick={() => setActiveTab('Monthly')} className={`inline-flex items-center justify-center p-4 rounded-t-lg cursor-pointer ${activeTab === 'Monthly' ? 'text-gray-900 border-b-4 border-gray-900' : 'text-gray-400 border-transparent'}`}
                                        aria-current={activeTab === 'Monthly' ? 'page' : undefined}>
                                        Monthly
                                    </a>
                                </li>
                                <li>
                                    <a onClick={() => setActiveTab('Yearly')} className={`inline-flex items-center justify-center p-4 rounded-t-lg cursor-pointer ${activeTab === 'Yearly' ? 'text-gray-900 border-b-4 border-gray-900' : 'text-gray-400 border-transparent'}`}>Yearly</a>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div class="flex mt-4 justify-center w-full bg-white border border-gray-500 rounded-lg shadow border-gray-400 ">
                            ...
                            <div ref={chartDivRef} style={{ width: "80%", height: "500px" }} ></div>
                        </div>
                    </div>
                </main>
            </div>
