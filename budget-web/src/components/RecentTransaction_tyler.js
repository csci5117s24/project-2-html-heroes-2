    ...
    return (
        <>
            <div className="ml-0 mr-0 mt-6 max-w-4xl p-6 bg-white border border-gray-400 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                <div className="flex">
                <h1 className="mb-6 text-4xl font-bold tracking-tight text-gray-900 dark:text-white">Recent Transactions</h1>
                </div>
                <ul className="h-40 overflow-y-auto mb-2 text-xl tracking-tight text-gray-400">
                    {transaction_list.map(each =>(
                        <li className="mb-4">
                        <div className="flex justify-between mb-2">
                            <span className={classes.dot} style={{backgroundColor: colorHash.hex(each.category)}}></span>
                            <p className="w-1/3 text-xl font-medium text-gray-500 truncate dark:text-white ">{each.category}</p>
                            <div className="w-1/3 font-medium text-gray-500 truncate dark:text-white" title={each.description}>{each.description}</div>
                            <p className="w-1/3 text-right font-medium text-gray-500 truncate mr-4 dark:text-white">-${parseFloat(each.value).toFixed(2)}</p>
                        </div>
                    </li>
                    )
                    )}
