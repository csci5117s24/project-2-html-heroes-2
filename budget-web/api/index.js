...
app.http('getUser', {
    ...
    handler: async (request, context) => {
        ...
        const user = await client.db('budgetapp').collection('users').findOne({_id: userId});
        return {
            jsonBody: {user: user}
        }
    },
...
app.http('addUser', {
    ...
    handler: async (request, context) => {
        ...
        const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
        const res = await client.db('budgetapp').collection('users').insertOne(newUser);
        client.close();
        return {
            status: 201,
...
app.http('setBudget', {
    ...
    handler: async (request, context) => {
        ...
        const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
        const result = await client.db('budgetapp').collection('users').updateOne(
            {_id: userId}, 
...
app.http('addCategory', {
    ...
    handler: async (request, context) => {
        ...
        const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
        const result = await client.db("budgetapp").collection("users").updateOne({_id: userId}, {$push: {categories: name}})
        client.close();
        if (result.matchedCount > 0) {
            return {
                status: 201
...
app.http('deleteCategory', {
    ...
    handler: async (request, context) => {
        ...
        const name = request.params.name;
        const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
        const result = await client.db("budgetapp").collection("users").updateOne({_id: userId}, {$pull: {categories: name}})
        client.close();
        if (result.matchedCount > 0) {
            return {
                status: 200
...
app.http('getTransaction', {
    ...
    handler: async (request, context) => {
        ...
        const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
        const transaction = await client.db('budgetapp').collection('transactions').findOne({_id: objectId, userId: userId});
        client.close();
        return {
...
app.http('getAllTransactions', {
    ...
    handler: async (request, context) => {
        ...
        const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
        const transactions = await client.db('budgetapp').collection('transactions').find({userId: userId}).toArray();
        client.close();
        return {
...
app.http('getAllCategoryTransactions', {
    ...
    handler: async (request, context) => {
        ...
        const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
        const transactions = await client.db('budgetapp').collection('transactions').find({userId: userId, category: category}).toArray();
        client.close();
        return {
            jsonBody: {transactions: transactions}
        }
